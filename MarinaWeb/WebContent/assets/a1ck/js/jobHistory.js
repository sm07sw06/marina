$(document).ready(function() {
	...
	function refreshData()  
	{
        ...
        var _SearchStat = $(":input:radio[name=F_STAT]:checked").val();
        ...
        var columns = [
	   		...
	   		{name:'JOB_TM'		 , width: 100, align : 'center',  
	   	   	 formatter : function(cellValue,rowObject,options) {
	   	   		 			//원래 값은 "python1" 일때
	   	   		 			return toDatePoint(cellValue);
	   	   		 			//화면에 보여지는 값은 python
	   	   	 			 }	
	   		}
	   	];
		...
		$.ajax({
			...
			success: function(json_data) {
		        ...
		        if(json_data.result == 'OK') {
			        ...
			        $('#F_SCAN_RECORD').val(toMoneyPoint(json_data.rows[0].SCAN_RECORD,0));
