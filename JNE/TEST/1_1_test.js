...
const IMG = require('../back-end/IMAGE');
        ...
        //회원기초정보관리
        {
            ...
            socket.on('update_data', function (msg) {
                ...
                if (newData) {
                    ...
                    app.post('/img/post', upload.single('image_Data_Param'), (req, res) => {

                        let COLUM = ['cctv_cd', 'send_time', 'boatinout'];
                        var object_search = {
                            cctv_cd: 'CCTV001',
                            send_time: '20210310120000',
                            boatinout: '1',
                        }
                        var object_save = {
                            photo: req.body.image_Data_Param,
                            photo_base64: req.body.image_Data_Param
                        }

                        var sqlQuery = SQL.AuToQuerySQL("UPDATE", "tb_cctvdata", COLUM, ['photo', 'photo_base64']);

                        COLUM.push('photo', 'photo_base64');

                        var sqlData = SQL.AuToSQLData(COLUM, object_save, object_search);
