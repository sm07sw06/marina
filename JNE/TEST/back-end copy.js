...
/*이미지 업로드*/
function imageUpload(files,path) {
...
function postgresSQL(sql, params = null, socketdata = []) {
    return new Promise(resolve => (
        client.query(sql, params, async (err, res) => {
...
io.on('connection', function (socket) {
    ...
    //회원기초정보관리
    {
        ...
        socket.on('search_data', function (msg) {
            ...
            //회원코드 일때
            if (parseInt(msg)) {
                var sql="SELECT * FROM tb_user_info WHERE marina_id = $1;";

                postgresSQL(sql,[msg],[socket,'search']).then(function(data){
                    console.log(data[0])
                }); 
                //DB 데이터 조회
        ...
        socket.on('save_data', function (msg) {
            ...
            app.post('/img/post', upload.single('new_image'), (req, res) => {
                ...
                imageUpload(req.file, imgfilename);
