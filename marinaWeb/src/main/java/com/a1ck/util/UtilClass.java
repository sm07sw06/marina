		...
		try {
			while (iter.hasNext()) {
				...
				if (item.isFormField()) {// �뙆�씪�씠 �븘�땶寃쎌슦
					...
					// 怨꾩냽 源⑥쭚
					String fileName = item.getName();// �븳湲� 怨꾩냽 源⑥쭚
					...
					if (!fileName.equals("") && fileName != null) {// �뙆�씪�씠由꾩씠 議댁옱�븯怨� �뾽濡쒕뱶�븷 �뙆�씪�씠 議댁옱�븳�떎硫� �뾽濡쒕뱶 �떆�옉

						int idx = fileName.lastIndexOf("\\"); // �쐢�룄�슦�쓽 寃쎌슦, c:\ 源뚯� �뼸湲�
						if (idx == -1) {
							idx = fileName.lastIndexOf("/"); // �쑀�땳�뒪(由щ늼�뒪)�쓽 寃쎌슦
						}
